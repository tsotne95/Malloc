...
//rough mesuring external and internal fragmentation in current state
//show the max possible availlable enough space for size of n
void mesure_fragmentation(size_t size)
{
  mem_free_block_t *f_block;
  mem_free_block_t *prev_f_block;
  mem_used_block_t *u_block;

  long long free_sum, max_free, full_used_space, full_payload, enough_for_n;
  free_sum = max_free = full_used_space = full_payload = enough_for_n = 0;
  ...
  if (first_free)
  {
    if ((unsigned char *)heap_start != (unsigned char *)first_free)
    {
      ...
      {
        full_payload += ((mem_used_block_t *)(unsigned char *)u_block)->size_without_alignment;
        full_used_space += get_aligned_size(MIN_USED_BLOCK_SIZE) + memory_get_allocated_block_size((unsigned char *)u_block + get_aligned_size(MIN_USED_BLOCK_SIZE));
    ...
    while (f_block)
    {
      free_sum += f_block->size + get_aligned_size(MIN_FREE_BLOCK_SIZE);
      max_free = MAX(max_free, f_block->size + get_aligned_size(MIN_FREE_BLOCK_SIZE));
      if (get_aligned_size(MIN_FREE_BLOCK_SIZE) + f_block->size >= MAX(get_aligned_size(size) + get_aligned_size(MIN_USED_BLOCK_SIZE), get_aligned_size(MIN_FREE_BLOCK_SIZE)))
        enough_for_n++;
  ...
  fprintf(stderr, "Total free  Max free  Ext.Frag  Total used  Total payload  Int.Frag  Enough Space\n");
  fprintf(stderr, "%10lld  %8lld  %8.2f  %10lld  %13lld  %8.2f  %11lld\n", free_sum, max_free, (free_sum) ? (free_sum - max_free) / ((double)free_sum) : 0, full_used_space, full_payload, (full_used_space) ? (full_used_space - full_payload) / ((double)full_used_space) : 0, enough_for_n);
}
...
void *memory_alloc(size_t size)
{
  ...
  //mesure fragmentation before alloc new block
  mesure_fragmentation(size);
  ...
  if (new_b->size >= MAX(get_aligned_size(MIN_USED_BLOCK_SIZE) + get_aligned_size(size), get_aligned_size(MIN_FREE_BLOCK_SIZE)))
  {
    ...
    used_b->size_without_alignment = size;
  }
  else
  {
